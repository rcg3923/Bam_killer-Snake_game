...
int printarr(int(*arr)[25], snake* f, WINDOW* win_score, WINDOW* win_mission, int grow, int poi)
{
    ...
    // 임시 배열 arr_map에 arr의 값을 복사합니다.
    // arr_map 배열은 실제 출력할 맵을 저장하기 위해 사용됩니다.
    for (int i = 0; i < 25; i++)
    {
        for (int j = 0; j < 25; j++)
        {
    ...
    int len = x->getLen();
    arr_map[x->getX()][x->getY()] = 3;
    for (int i = 1; i < len; i++)
    {
        x = x->point;
        arr_map[x->getX()][x->getY()] = 4;
    }

    // 임시 배열 arr_map을 출력합니다.
    ...
    for (int i = 0; i < 25; i++)
    {
        for (int j = 0; j < 25; j++)
        {
            ...
            else if (arr_map[i][j] == 2)
            {
                attron(COLOR_PAIR(3));
                mvprintw(i, j * 3, "000");
                attroff(COLOR_PAIR(3));
            }
            else if (arr_map[i][j] == 3)
            {
                attron(COLOR_PAIR(4));
                mvprintw(i, j * 3, "000");
                attroff(COLOR_PAIR(4));
            }
            else if (arr_map[i][j] == 4)
            {
                attron(COLOR_PAIR(5));
                mvprintw(i, j * 3, "000");
                attroff(COLOR_PAIR(5));
            }
            else if (arr_map[i][j] == 10)
            {
                attron(COLOR_PAIR(6));
                mvprintw(i, j * 3, "+++");
    ...
    mvwprintw(win_mission, 6, 1, " G : %d  (%c)", gG, chk_G);
    wborder(win_mission, '*', '*', '*', '*', '*', '*', '*', '*');
...
int check(int (*arr)[25], snake * new_temp, int input)
{
    // 맵 확인을 위한 배열
    int table[25][25] = {0};
    // 머리 위치 변수
    ...
    // 뱀의 머리 위치 저장
    head_x = new_temp->getX();
    head_y = new_temp->getY();
    // 뱀의 길이 저장
    int len = new_temp->getLen();

    // 뱀의 몸통 부분을 check_map에 표시
    for (int i = 1; i < len - 1; i++)
    {
        new_temp = new_temp->point;
        table[new_temp->getX()][new_temp->getY()] = 4;
    }
    ...
    case KEY_UP:
        ...
        if (table[head_x - 1][head_y] == 1 || table[head_x - 1][head_y] == 4)
            return 0;
        // 목표 지점에 도달한 경우
        else if (table[head_x - 1][head_y] == 10)
            return 2;
        // 적절한 이동인 경우
        else if (table[head_x - 1][head_y] == -10)
            return 3;
    ...
    case KEY_LEFT:
        // 벽 또는 뱀의 몸통과 부딪힌 경우 이동 불가능
        if (table[head_x][head_y - 1] == 1 || table[head_x][head_y - 1] == 4)
            return 0;
        // 목표 지점에 도달한 경우
        else if (table[head_x][head_y - 1] == 10)
            return 2;
        // 적절한 이동인 경우
        else if (table[head_x][head_y - 1] == -10)
            return 3;
...
void creat_item(int (*arr)[25], snake *a, item *it)
{
    // 맵 상태를 확인하기 위한 임시 배열
    int temp[25][25] = {0};
    // 뱀 객체 복사
    ...
    r = rand() % 2;
    if (r == 0)
        it->condition = 10; // 10은 양수 아이템을 나타냄
    else
        it->condition = -10; // -10은 음수 아이템을 나타냄
    ...
    arr[x][y] = it->condition; // 아이템 위치에 상태 설정
}
